...
const AttendanceTeacherCard = () => {
  const { teachers, aToken, getAllTeachers } = useContext(AdminContext);
        ...
        <table className="min-w-full table-auto border-collapse mt-5">
          <thead>
            <tr className="border-b bg-gray-100">
              <th className="px-4 py-2 text-left text-gray-600">Teacher ID</th>
          ...
          <tbody>
            {filteredTeachers.map((teacher, index) => (
              <tr key={teacher.teacherId} className={`hover:bg-gray-50 ${index === filteredTeachers.length - 1 ? '' : 'border-b'}`}>
                <td className="px-4 py-2">{teacher.teacherId}</td>
                <td className="px-4 py-2">{teacher.name}</td>
                <td className="px-4 py-2">
                  {isViewingTimeIn ? teacher.timeIn : teacher.timeOut}
                </td>
              </tr>
            ))}
          </tbody>
