...
const AddAdministrator = () => {
  ...
  const [position, setPosition] = useState('Academic Coordinator');
  ...
  const onSubmitHandler = async (event) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        setAddress1('');
        setAddress2('');
        setPosition('Academic Coordinator');
        setNumber('');
  ...
  return (
    <div className="flex justify-center items-center min-h-[90vh] w-full">
      <form onSubmit={onSubmitHandler} className="w-full max-w-5xl bg-white p-6 rounded-lg shadow-lg">
        <h2 className="text-xl font-semibold mb-4 text-center">Add Administrator</h2>
        ...
        <div className="space-y-3">
          ...
          <select onChange={(e) => setPosition(e.target.value)} value={position} className="border rounded px-3 py-2 w-full">
